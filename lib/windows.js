...
class Windows {
    ...
    async settings() {
        ...
        console.log(kleur.yellow().bold('\nSettings successfully saved! New settings take effect immediately!'));
    ...
    async playlist() {
        ...
        if (response && response.value) {
            console.log(kleur.yellow().bold(`\nPlaying the track "${response.value.title}"`))
...
function suggest(a, b) {
    ...
    let resp = [];
    for (let d of b) {
        if (d.search.indexOf(toLower(a)) > -1) {
            resp.push(d)
        }
    }
