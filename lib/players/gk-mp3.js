        ...
        // player stats
            ...
            str =
                ...
                c.gray(
                    ...
                    // Loop Enabled?
                    (this.flags.loop ? "LOOPS Ҩ | " : "") +
                    // total play time
                    `PLAYTIME: ${prettyMs(this.totalPlayedTime,{ colonNotation: true })}  `
                )
        }
        // track stats
...
class Player extends Loghelper {
    ...
    current_video() {
        ...
        if (this.trackNum < this.videos.length) {
            ...
            // check with last status
            // if not shuffle then simply skip to last played file
            // this continue from last session only works when shuffle is not enabled
            if (!this.flags.shuffle && this.trackNum === 0 && this.settings.playback.continueLastSession == 'yes') {
                this.trackNum = this.playSession.currentTrackNum;
            }
    ...
    async play(videos) {
        ...
        // 
        EE.on('play-next-track', run_playlist);
        EE.on('update_player', update_player)
        ...
        EE.on("finished-playback", function() {
            // exit procedures...
            // close process
            if (self.ffplaySubProcess) {
                self.ffplaySubProcess.cancel();
            }
            ...
            setTimeout(() => {
                ...
                console.log('>> Finished! Nothing more to do here. Adios!\n');
            }, 1000);
    ...
    async play_audio() {
        ...
        // play...
        this.ffplaySubProcess = execa(this.ffplayPath, args);
        ...
        this.ffplaySubProcess.stdout.on('data', (data) => {
            // console.log(data.toString());
        });

        let line, time, dataArr, matches;

        this.ffplaySubProcess.stderr.on('data', async(data) => {
        ...
        this.ffplaySubProcess.stderr.on('end', (data) => {
            self.isPlaying = false;
            ...
            if (this.isOnline && !self.attemptingAudioFallback && !self.errorStatus) {
                ...
                //  this.playSession = playCache.get('playSession') || {played:[],currentTrackNum:0};
                // keep data sizeable
