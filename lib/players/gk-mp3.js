    ...
    format_text(type, formatLine = true, rtl = true) {
        ...
        // the track line
            ...
            str = kleur.bold(
                ...
                ` ${this.title_truncate(dotProp.get(this,"currentSource.title",''))}`
            )
        }
        // player stats
            ...
            str =
                ...
                kleur.gray(
                    ...
                    // Shuffled?
                    (dotProp.get(this, "flags.shuffle", false) ? "SHUFFLED Ջ | " : "") +
                    // Loop Enabled?
                    (dotProp.get(this, "flags.loop", false) ? "LOOPED Ҩ | " : "") +
                    // total play time
        ...
        // track stats
            ...
            let timeDiff = dotProp.get(this, "currentSource.duration", 0) ? (dotProp.get(this, "currentSource.duration", 0) - this.trackTime) : this.trackTime;

            let formattedTime = prettyMs(timeDiff, { colonNotation: true }).split(':')
                .map(n => String(Math.round(n)).padStart(2, '0')).join(':');

            let views = numbro(dotProp.get(this, "currentSource.views", 0)).format({ average: true, mantissa: dotProp.get(this, "currentSource.views", 0) > 1000 ? 1 : 0 }),
                audioBitrate = dotProp.get(this, 'currentAudio.audioBitrate', null)

            str = (dotProp.get(this, "currentSource.duration", null) ?
                    ` ${this.progress_bar(this.trackTime)}` :
                    ` [` + 'x'.repeat(20) + `] |`) +
                ` ${formattedTime} |` +
                (audioBitrate ? audioBitrate + 'kbps |' : '') +
                ` ${bytes(Number(dotProp.get(this,'currentAudio.contentLength',)))}` +
                (dotProp.get(this, "currentSource.views", 0) ? ` | ${views} YT Views` : '')
        }
...
class Player extends Loghelper {
    ...
    async listen_for_events() {
        ...
        this.EE.on('play-next-track', next_track, this);
    ...
    async check_for_updates() {
        ...
        let currentVersion = await latestVersion('geekplay');
        if (semver.lt(pkgData.version, currentVersion)) {
            this.belowWindowMsg = kleur.yellow().bold(`There's a new version of GeekPlay. Please update to v${currentVersion} -> npm update -g geekplay`)
        }
    ...
    async run_playlist() {
        ...
        if (hasCurrentSource) {
            ...
            // we have finished playback?
            this.EE.emit("finished-playback");
        }
