...
class Player extends Loghelper {
    ...
    async update_player(time) {
        ...
        if (!this.painted) {
            ...
            let headerLine = `\n  Queued ` + c.bold(`${numeral(this.totalTracks).format('0a')} Tracks`) + ` | ` +
                `Total Playtime: ` + c.bold(`${prettyMs(this.totalPlayTime, { verbose: true })}`),
        ...
        let content =
            ...
            this.format_line(
                c.gray.italic(
                    ` Next Track: ` + this.title_truncate(nextTrack, 15) +
    ...
    async play_audio() {
        ...
        child.stderr.on('data', (data) => {
            ...
            if (time) {
                ...
                if (pc >= .95 && !self.nextAudioLoading) {
    ...
    async get_audio(currentVideo) {
        ...
        if (!results) {
            ...
            cache.set('audio', currentVideo.url, audioFormats);
        } else {
            if (!this.painted) {
