...
const ytdl = require('ytdl-core'),
    ...
    Conf = require('conf'),
    { duration_to_ms, stripAnsi, truncate_to_window } = require('../common');
...
class Loghelper {
    ...
    format_text(type, formatLine = true, rtl = true) {
        ...
        if (type == 'header') {
            ...
            str = c.gray(`${prettyMs(this.totalPlayedTime,{ colonNotation: true })} `)
...
class Player extends Loghelper {
    ...
    async update_player(time) {
        ...
        let content =
            ...
            (this.nextAudioLoading || this.nextAudioReady ? this.format_text('loading-next-track') : "") +
            // blank spacing line
            this.format_line('') +
            // bottom border line
            this.format_text('borderline-bottom');
