...
class Loghelper {
    ...
    progress_bar(time) {
        ...
        let pcSize, procgressChar = '■',
            pc, str = '',
            size = Math.ceil(this.consoleRows / 4);
    ...
    format_text(type, formatLine = true, rtl = true) {
        ...
        // player stats
            ...
            str =
                ...
                c.gray(
                    // Volume 
                    ('VOL ͽ' + ')'.repeat(Math.ceil(volumePC * 5))).padStart(5) + ' | ' +
                    // Shuffled?
...
class Player extends Loghelper {
    ...
    async play_audio() {
            ...
            if (msgType !== 'info' && httpError) {
                ...
                if (matches) {
                    ...
                    if (pc >= .8) {
                        ...
                        if (!self.nextAudioLoading && !this.nextAudioReady) {
        ...
        this.ffplaySubProcess.stderr.on('end', (data) => {
            ...
            if (!this.attemptingAudioFallback) {
                // try play next ....
                EE.emit('file-playback-finished');
            }
        })
