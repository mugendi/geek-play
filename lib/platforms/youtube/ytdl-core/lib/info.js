...
const getWatchHTMLPageBody = (id, options) => {
    ...
    return exports.watchPageCache.getOrSet(url, () => got(url, options.requestOptions).then(resp => resp.body));
};
...
const getEmbedPageBody = (id, options) => {
    ...
    return got(embedUrl, options.requestOptions).then(resp => resp.body);
};
    ...
    while (currentTry <= options.maxRetries) {
        try {
            ...
            if (err instanceof UnrecoverableError ||
                (err.statusCode < 500) || currentTry >= options.maxRetries) {
                throw err;
            }
...
const getWatchJSONPage = async(id, options) => {
    ...
    let body = await got(jsonUrl, reqOptions).then(resp => resp.body);
...
const getVideoInfoPage = async(id, options) => {
    ...
    let body = await got(url.toString(), options.requestOptions).then(resp => resp.body);
